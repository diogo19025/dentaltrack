...
Tokens em `app/globals.css` (`@theme`): cores shadcn, `--chart-1..5` (mais `6..8`, só para colorir a equipe na agenda), `--tag-*`, status, `--radius` 0.7rem, sombras `xs..xl`. Fontes Geist e Geist Mono, classe `tabular` para números. Acessibilidade AA: rótulos ligados a campos, setas em tablist, `role=log` no chat, foco visível, `prefers-reduced-motion`.
...
- **Na `/agenda`, a cor é sempre a do profissional.** Houve um seletor "por profissional · por procedimento" (2026-09-18 → 09-19); saiu porque a recepção precisa ver de relance *quem* atende, e o procedimento já está escrito no bloco e no filtro. Posição no cadastro → paleta de charts (oito cores; com cinco, dez profissionais repetiam cor na mesma grade), estável quando alguém entra ou sai. Legenda só com equipe de 2+, e ela é filtro rápido: clicar num nome isola o profissional.
- **A grade é de segunda a domingo, e o bloco é um tom claro com a barra do profissional.** A grade começava no dia de hoje, então o intervalo mudava todo dia e não havia "a semana" para o olho se apoiar; passou para a semana fixa dos calendários que a recepção já usa (2026-09-19), com o passado esmaecido. O bloco sólido com texto branco a 10px não passava no contraste nas cores claras da paleta e virava uma parede de chips numa semana cheia; o tom claro com o texto na cor escurecida é o desenho do Apple/Google Calendar. Horários que coincidem ficam lado a lado (com dez profissionais, dois às 09:00 é o caso comum; antes um cobria o outro). A integração conectada é uma linha de status, não uma faixa: o estado normal não compete com o conteúdo.
