...
- **LGPD operacional** — anonimização dos dados pessoais a pedido do titular (o histórico e as métricas ficam), descadastro visível e alternável no painel do lead, e retenção opcional da fila de saída.
...
| **Agendamentos do período**       | Registrados × de pé × cancelados.            | `Appointment` no recorte; cancelados por `canceledAt` (pega os marcados antes).|
...
**Cancelar não desfaz a conversão — de propósito.** Quando um agendamento é cancelado, a conversa que o originou continua `agendada`: ela converteu, o agente fez o trabalho, e `agendada` é estado terminal. O que o dashboard passou a mostrar (2026-09-16) é a outra metade da informação, ao lado do funil: **agendamentos registrados × ainda de pé × cancelados no período**. População diferente (agendamentos, não conversas), por isso os números não batem com o funil.
...
| 2026-09-16 | **As duas pendências da F14 fechadas** ([`PENDENCIAS.md`](PENDENCIAS.md)): (1) o dashboard passou a mostrar, sob o funil, **quantos agendamentos o período gerou, quantos continuam de pé e quantos foram cancelados** (`metrics.bookings`). A distorção era real — desde que o cliente desmarca sozinho pelo WhatsApp, a conversão contava conversas que já não tinham consulta —, mas a correção ficou na **leitura**: a máquina de status da conversa, o funil, o scoring e o histórico de `daily_metric` continuam contando o que sempre contaram. (2) `findMyAppointments` deixou de declarar objeto de parâmetros vazio (ganhou o `motivo` opcional): o Gemini recusa `properties: {}` com 400, e ele é o **fallback** — a tool derrubaria o plano B sem sintoma próprio. Um teste passou a proibir schema vazio em qualquer tool futura. Sem migration. |
| 2026-09-16 | **Envio de arquivos passou a funcionar de verdade.** A `SUPABASE_SERVICE_ROLE_KEY` estava **declarada e vazia** desde a F13 — a API sobe igual, e só quem clicava em "Enviar logo" descobria, pelo 503. Chave preenchida no ambiente local e no Railway, validada ponta a ponta (bucket `media` público criado, upload, URL pública lida **sem credencial** — que é como a Evolution busca a mídia —, objeto de teste apagado) e o upload de logo confirmado em produção. Para a falta deixar de ser silenciosa, `GET /health` passou a reportar `arquivos`, do mesmo jeito e pelo mesmo motivo que já reportava `whatsapp`: pelo `isConfigured()`, sem nenhuma chamada externa no healthcheck. |
