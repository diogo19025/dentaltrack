...
function exec(tool: unknown, input: unknown): Promise<any> {
  const fn = (tool as { execute: (i: unknown, o: unknown) => Promise<unknown> }).execute;
...
describe("buildChatTools", () => {
  ...
  it("searchProcedures: retorna procedimentos da clínica com preço formatado", async () => {
    ...
    expect(prismaMock.procedure.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ clinicId: CLINIC_ID, active: true }) }),
    );
  ...
  it("captureLead: cria lead novo e vincula à conversa", async () => {
    ...
    const res = await exec(tools().captureLead, { nome: "João", telefone: "11999990000" });

    expect(res).toEqual({ ok: true, leadId: "lead-1" });
    expect(prismaMock.lead.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ clinicId: CLINIC_ID, name: "João", phone: "11999990000" }),
      }),
    );
  ...
  it("captureLead: atualiza o lead existente da conversa", async () => {
    prismaMock.conversation.findFirst.mockResolvedValueOnce({ leadId: "lead-x" });
  ...
  it("bookAppointment: cria appointment e marca conversa como agendada", async () => {
    ...
    expect(conversationsMock.markAsScheduled).toHaveBeenCalledWith(CONVERSATION_ID, CLINIC_ID);
  });
  ...
  it("bookAppointment: agenda mesmo se a transição de status falhar", async () => {
    ...
    conversationsMock.markAsScheduled.mockRejectedValueOnce(new Error("transição inválida"));
