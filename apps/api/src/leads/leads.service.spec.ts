...
describe("LeadsService", () => {
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [LeadsService, { provide: PrismaService, useValue: prismaMock }],
    }).compile();
  ...
  it("deriva interesse (procedimento), tags distintas e status da conversa mais recente", async () => {
    ...
    expect(res.summary).toEqual({ total: 2, agendada: 1, andamento: 1, abandonada: 0 });
    ...
    expect(res.leads[1]).toMatchObject({ name: "Maria", interest: null, status: "em_andamento" });
  });
    ...
    prismaMock.lead.findMany.mockResolvedValueOnce([
      {
        ...
        createdAt: new Date(),
        conversations: [
          { status: "abandonada", conversationTags: [{ tag: { name: "clareamento", color: "amber" } }] },
        ],
        appointments: [],
      },
    ]);
