...
function buildPrismaMock() {
  ...
  return {
    ...
    conversationTag: {
      groupBy: jest.fn().mockResolvedValue([{ tagId: T1, _count: { tagId: 3 } }]),
    },
    tag: {
      findMany: jest.fn().mockResolvedValue([{ id: T1, name: "implante", color: "teal" }]),
    },
  };
}
...
describe("MetricsService", () => {
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [MetricsService, { provide: PrismaService, useValue: prismaMock }],
    }).compile();
