...
import { type EnsureClinicResult, OnboardingService } from "./onboarding.service";
...
export class OnboardingController {
  ...
  bootstrap(@CurrentUser() user: AuthUser): Promise<EnsureClinicResult> {
    const meta = user.raw.user_metadata as { clinic_name?: unknown } | undefined;
    const clinicName = typeof meta?.clinic_name === "string" ? meta.clinic_name : undefined;
    return this.onboarding.ensureClinic({ userId: user.id, email: user.email, clinicName });
  }
}
