 ...
 * `clinicId` do usuário (via Membership). A entrada é texto OU áudio (base64,
 * transcrito no servidor — a transcrição volta no header `X-Transcript`). A
 * resposta é um UI message stream do AI SDK (consumível pelo `useChat`); o
 * `conversationId` volta no header `X-Conversation-Id`.
 */
@Controller('chat')
